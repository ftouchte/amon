...
int main(int argc, char const *argv[]) {
    ...
    gr_ahdcView->SetTitle(TString::Format("AHDC View at z = %.2lf; x(mm); y(mm)", zpos).Data());
    ...
    while( reader.next()){
        ...
        for (int trackRow = 0; trackRow < trackBank.getRows(); trackRow++) {
            // Processing
            TCanvas * cn = new TCanvas(TString::Format("c%d_%d", (int) nevents, trackRow+1).Data(),TString::Format("c%d_%d ahdcView", (int) nevents, trackRow+1).Data(), 1000, 1000);
            cn->cd();
            // Draw AHDC view
            gr_ahdcView->Draw("AP");
            ...
            gr_hits->Draw("P");
            // key : Niter, orientation, status --> TGraph*
            std::map<std::tuple<int, int, int>, TGraph*> graphMap;
            for (int i = 0; i < kfmonBank.getRows(); i++) {
                ...
                // it : iterator, it is an std::pair containing 
                ...
                //if (Niter > 2) continue;
                auto [it, inserted] = graphMap.insert({std::make_tuple(Niter, orientation, status), new TGraph()});
            ...
            for (auto& [k, g] : graphMap) {
                ...
                // Set Color
                if (Niter < 8) {
                    g->SetLineColor(Niter+1);
                    g->SetMarkerColor(Niter+1);
                }
                else { // Niter >= 9
                       // we ship the kColor 10 as it is transparente
                    g->SetLineColor(Niter+2);
                    g->SetMarkerColor(Niter+2);
                }
                // Set Marker style
                if ((orientation % 2 == 0) && (status == 0)) {
                    // forward or postfit propagation and prediction
                    g->SetMarkerStyle(26);
                    legend->AddEntry(g, TString::Format("Niter : %d, %s, %s", Niter, "forward", "prediction").Data());
                }
                else if ((orientation % 2 == 0) && (status == 1)) {
                    // forward or postfit propagation and correction
                    g->SetMarkerStyle(22);
                    legend->AddEntry(g, TString::Format("Niter : %d, %s, %s", Niter, "forward", "correction").Data());
                }
                else if ((orientation % 2 == 1) && (status == 0)) {
                    // backward propagation and prediction
                    g->SetMarkerStyle(32);
                    legend->AddEntry(g, TString::Format("Niter : %d, %s, %s", Niter, "backward", "prediction").Data());
                }
                else { //if ((orientation % 2 == 0) && (status == 1)) {
                    // backward propagation and correction
                    g->SetMarkerStyle(23);
                    legend->AddEntry(g, TString::Format("Niter : %d, %s, %s", Niter, "backward", "correction").Data());
                }
                cn->cd();
                g->Draw("same PL");
                //legend->Draw();
